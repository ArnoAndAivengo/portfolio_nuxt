...
import type { QuizLesson, QuizLevel, QuizView, LessonResult, ShuffledOption } from './types'

const PASS_PCT = 80
export const QUIZ_LETTERS = ['A', 'B', 'C', 'D', 'E'] as const
...
export const useLanguageQuiz = <L extends string>(options: Options<L>) => {
  const { lessons, levels, storage, cookie, level, resetConfirm } = options
  const isLevelId = (value: unknown): value is L =>
  ...
  const setLevel = (id: string) => {
    if (!isLevelId(id) || level.value === id) return
    level.value = id
    goHub()
  }
  ...
  const onKey = (event: KeyboardEvent) => {
    ...
    const fromLetter = QUIZ_LETTERS.findIndex((item) => item === letter)
