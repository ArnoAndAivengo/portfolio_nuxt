---
name: Александр Обухов
role: Frontend Engineer
status: Открыт к предложениям
eyebrow: Портфолио
lead: Фокус на Vue 3 / Nuxt.
experienceYears: 7+ лет
updated: 7 сентября 2026
seoTitle: Александр Обухов — Frontend Engineer
seoDescription: "Frontend Engineer, 7+ лет: Vue 3, Nuxt, TypeScript, FSD. E-commerce, аналитика, edtech. Удалёнка, гибрид или офис в Москве."
ogImage: https://arnoandaivengo.ru/images/alex/alex-og.jpg
canonical: https://arnoandaivengo.ru/
telegram: https://t.me/alexObukhovArno
email: alexobukhovarno@gmail.com
phone: +7 915 212‑77‑21
phoneHref: tel:+79152127721
maxUrl: https://max.ru/?phone=79152127721
github: https://github.com/ArnoAndAivengo
gitlab: https://gitlab.com/ArnoAndAivengo
skills:
  - label: Frontend
    items: [Vue 3, Nuxt 3, TypeScript, FSD, React, Pinia, Vite, Tailwind, ECharts, Storybook]
  - label: Backend
    items: [Node.js, NestJS, Laravel, PostgreSQL, Redis, JWT]
  - label: Поставка
    items: [Docker, Nginx, GitLab CI, Vitest, Playwright]
---

**Frontend Engineer с 7+ лет** коммерческого опыта в e‑commerce, аналитике и edtech.
Специализируюсь на сложных SPA и SSR: проектирую клиентскую архитектуру, 
держу качество релиза и довожу интерфейс до измеримого бизнес-результата.

Лучше всего работаю на стыке продукта, дизайна и инженерии: 
Vue 3 / React / TypeScript, FSD, стандарты команды и CI/CD. 
При необходимости закрываю смежный контур frontend → backend → DevOps 
(Node.js, FastAPI, Docker, Nginx) — без смены основной экспертизы с фронта.

Рассматриваю роль Senior Frontend (Vue / Nuxt) — полная удалёнка, гибрид или офис в Москве.
